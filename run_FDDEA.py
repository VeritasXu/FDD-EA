...
from utils.data_utils import index_bootstrap, sort_data
    ...
    for prob in problems:
        for d in ds:
            ...
            for IR in range(Max_IR):
                ...
                if args.init_type == 'same':
                    # type 'same' is for fair comparison with centralized methods
                    ...
                    record_profiles[0:num_data, IR] = copy_y[copy_index, :].flatten()

                    for i in range(N):
                        # default boot_prob = 1 means that no bootstrap
                        data_index = index_bootstrap(num_data, boot_prob)
                        D_samp[i], D_label[i] = train_x[data_index], train_y[data_index]

                        if alpha != 0.0:
                            D_label[i] += alpha * np.random.randn(num_data, M)


                elif args.init_type == 'diff':
                    for i in range(N):
                        D_samp[i], D_label[i] = init_samples(func=Function,
                                                             num_b4_d=num_b4_d, d=d,
                                                             xlb=lb, xub=ub)
                else:
                    print('Please choose initial type from {same, diff}!')
                ...
                while Real_FE < 11 * d:
                    ...
                    for idx in idx_users:
                        ...
                        if Dl_samp[idx].shape[1] != d:
                            ...
                            if alpha != 0.0:
                                Dl_label[idx] += alpha * np.random.randn()

                        else:
                            tmp_label = best_fit
                            if alpha != 0.0:
                                tmp_label += alpha * np.random.randn()

                            Dl_samp[idx] = np.vstack((Dl_samp[idx], chosen_pop))
                            Dl_label[idx] = np.vstack((Dl_label[idx], tmp_label))

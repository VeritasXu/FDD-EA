...
class Client:
    ...
    def find_center(self, train_x):
        ...
        # std = np.tile(Spreads, (1, self.k)).flatten()
        # an alternative way for spreads
        # std = np.repeat(dMax / np.sqrt(2 * self.k), self.k)
        # print(centers.shape)
        # print(std.shape)
        return centers, std
    ...
    def compute_update(self):
        ...
        return tmp_c, tmp_w, tmp_b, tmp_s

    def reload(self, m_tuple):
        tmp_c, tmp_w, tmp_b, tmp_s, self.nk = m_tuple[0], m_tuple[1], m_tuple[2], m_tuple[3], m_tuple[4]
        self.centers = copy.deepcopy(tmp_c)
        self.w = copy.deepcopy(tmp_w)
        self.b = copy.deepcopy(tmp_b)
        self.std = copy.deepcopy(tmp_s)
